...
class Themify_Customizer {
	...
	function __construct() {
		...
		define( 'THEMIFY_CUSTOMIZER_URI', THEMIFY_BASE_URI . '/customizer' );
		define( 'THEMIFY_CUSTOMIZER_DIR', THEMIFY_BASE_DIR . '/customizer' );
	...
	function customize_control_scripts() {
		// Font Icon CSS
		wp_enqueue_style( 'themify-icons', THEMIFY_CUSTOMIZER_URI . '/css/themify-icons.css', array(), THEMIFY_VERSION );
			...
			if ( 'font' == $style || 'logo' == $style || 'tagline' == $style || 'decoration' == $style ) {
				...
				if ( isset( $font->family->name ) && '' != $font->family->name ) {
					if ( isset( $font->family->fonttype ) && 'google' == $font->family->fonttype ) {
						...
						$font_hash = $font->family->name . $font->family->variant . implode(',', $user_subsets);
						wp_enqueue_style( 'custom-google-fonts-' . md5($font_hash), themify_base_https_esc( 'http://fonts.googleapis.com/css' ) . '?family=' . str_replace( ' ', '+', $font->family->name ) . ':' . $font->family->variant . '&subset=' . implode(',', $user_subsets) );
					}

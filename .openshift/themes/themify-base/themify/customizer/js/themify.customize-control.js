...
(function( exports, $ ){
	...
	api.ThemifySubAccordion = api.Control.extend({
		ready: function() {
			...
			$navMenu.closest('label').map(function( index ) {
	...
	api.ThemifyControl = api.Control.extend({
		...
		isWebFont: function(f) {
			var arr = [', sans-serif', ', serif'],
				contains = false;
		...
		ready: function() {
			...
			if ( '' != $field.val() ) {
				...
				try {
					...
					if ( control.isWebFont( currentVal ) ) {
						control.value[control.id] = { 'family': {
							'name': currentVal,
							'fonttype': 'websafe'
						}};
					} else {
		...
		ready: function() {
			...
			if ( '' != $field.val() ) {
				...
				try {
					...
					if ( control.isURL( currentVal ) ) {
						control.value[control.id] = { 'src': currentVal };
					} else {
						control.value[control.id] = {};
					}
				}
	...
	api.ThemifyCustomCSS = api.ThemifyControl.extend({
		...
		expand: function( container, $customCSS ) {
			...
			$('.themify-expand', container).on('click', function(e){
				...
				$expand.find('.customcss').on('keyup', function() {
				...
				$expand.find('.themify-contract').on('click', function(e){
					...
					$expand.find('.customcss').off('keyup');
